 ...
 *                                                                           *
\*===========================================================================*/
#ifndef ZERNIKEMOMENTS_CPP
#define ZERNIKEMOMENTS_CPP
...
template<class VoxelT, class MomentT>
void ZernikeMoments<VoxelT,MomentT>::SetZMRef(std::vector<ComplexT> zmRef) {
...
template<class VoxelT, class MomentT>
MomentT ZernikeMoments<VoxelT,MomentT>::GetMomentVariation(double x, double y, double z, T xCOG, T yCOG, T zCOG, T scale) {
    ...
    // precompute the combination of xyz powers
    std::vector<std::vector<std::vector<double> > > xyzPower;
    double xPower, yPower, zPower;

    xyzPower.resize(order_+1);
    int xp,yp,zp;
    for(int n = 0; n <= order_; n++) {

        if(n > 0) {xPower *= dsx;xp++;}
        else {xPower = 1.0;xp = 0;}

        xyzPower[n].resize(order_ + 1);
        for(int l = 0; l <= order_; l++) {

            if(l > 0) { yPower *= dsy;yp++;}
            else {yPower = 1.0;yp = 0;}

            xyzPower[n][l].resize(order_ + 1);
    ...
    for (int n=0; n <= order_; ++n)  {
        ...
        for (int l = l0; l<=n; ++li, l+=2) {
            for (int m=0; m<=l; ++m) {
                // Zernike moment of according indices [nlm]
                ComplexT sumIn ((T)0, (T)0);

                int nCoeffs = (int)gCoeffs_[n][li][m].size ();
                for (int i=0; i<nCoeffs; ++i) {
                    dGM = 0.0;
                    cc = gCoeffs_[n][li][m][i];
    ...
    return -sumOut; // take minus b/c phi is negative inside
}

template<class VoxelT, class MomentT>
MomentT ZernikeMoments<VoxelT,MomentT>::GetZernikeDistance() {
