 ...
 *                                                                           *
\*===========================================================================*/
#ifndef ZERNIKDESCRIPTOR_CPP
#define ZERNIKDESCRIPTOR_CPP
...
template<class T, class TIn>
void ZernikeDescriptor<T, TIn>::SetZMReference(std::vector<ComplexT> zmRef) {

    zm_.SetZMRef(zmRef);
}

template<class T, class TIn>
void ZernikeDescriptor<T, TIn>::ComputeZernikeMoments (T* _voxels, int _dim, int _order){
...
template<class T, class TIn>
T ZernikeDescriptor<T, TIn>::GetZernikeDistance(){

    return zm_.GetZernikeDistance();
}
...
template<class T, class TIn>
void ZernikeDescriptor<T,TIn>::SaveMoments (const char* fnSave)  {
    ...
    for (int n=0; n<order_+1; ++n) {
        ...
        for (int l = n % 2; l<=n; ++li, l+=2) {
            for (int m=0; m<=l; ++m) {
                ...
                outfile << std::real(moment) << " " << std::imag(moment) << std::endl;
            }
        }
    }
...
template<class T, class TIn>
void ZernikeDescriptor<T,TIn>::GetZernikeMoments(std::vector<float>& zm_vec) {
    ...
    for (int n=0; n<order_+1; ++n) {
        ...
        for (int l = n % 2; l<=n; ++li, l+=2) {
            for (int m=0; m<=l; ++m) {
                ...
                zm_vec.push_back(std::real(moment));
                zm_vec.push_back(std::imag(moment));
            }
        }
    }
}
...
template<class T, class TIn>
T ZernikeDescriptor<T,TIn>::GetMomentVariation(double x, double y, double z) {

    return zm_.GetMomentVariation(x, y, z,xCOG_, yCOG_, zCOG_, scale_);
}
