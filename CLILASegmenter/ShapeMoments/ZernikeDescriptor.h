...
public:
    ...
    void SaveMoments(const char* fnSave);
    /// Access to invariants
    T2D GetInvariants ();

    /**
      These functions were added to the original code to compute the variations of the Zernike moments
    */
    void SetZMReference(std::vector<ComplexT> zmRef);
    T GetMomentVariation(double x, double y, double z);

    void ComputeZernikeMoments (T* _voxels, int _dim, int _order);

    T GetZernikeDistance();
    void GetZernikeMoments(std::vector<float>& zm_vec);
...
private:
    ...
    T       zeroMoment_,            // zero order moment
            x1Moment_,              // 1st order moment for x
            y1Moment_,              // 1st order moment for y
            z1Moment_,              // 1st order moment for z
