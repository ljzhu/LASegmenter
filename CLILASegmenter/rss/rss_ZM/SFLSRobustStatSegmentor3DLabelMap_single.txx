...
::computeForce()
{
//  double zmMax = std::numeric_limits<double>::min();
...
//      zmMax = fmax>fabs(zmForce[i])?fmax:fabs(zmForce[i]);
...
//        KappaNorm += kappaOnZeroLS[i];
    }
    //  mean = sum/n;
    //  sigma = std::sqrt((sum2 - mean*mean)/n);
      ZMNorm = std::sqrt(ZMNorm);
//      KappaNorm = std::sqrt(KappaNorm);
...
//      this->m_force[i] = cvForce[i]/(sigma + 1e-10);

//      std::cout << i << ":" << this->m_force[i] << std::endl;
    }
...
::doSegmenation()
{
  ...
  std::ofstream fZM("../../Results/Test/ZM.m");
  fZM<< "ZMDist = [";
...
// #endif
  ...
  double eVal, eValMed, eValNxt, eValFirst, eValMax = 0.0;
...
//       #endif
      ...
      std::cout << "ZM Distance [" << it << "]=" << m_ZM.GetZernikeDistance() << std::endl;
...
//      fZM << (1 - (this->m_curvatureWeight))*m_ZM.GetZernikeDistance() + \
//             (this->m_curvatureWeight)*std::pow(this->m_lz.size(),3)/std::pow(this->m_insideVolume, 2) << " ";
      fZM << m_ZM.GetZernikeDistance() << " ";
      // Compute Forces
...
//    std::cout << "size: " << size << ";" << "origin " << m_ZMOrigin[0] << "," << m_ZMOrigin[1] << "," << m_ZMOrigin[2] << std::endl;
//    std::cout << "origin: " << m_ZMOrigin[0] << "," << m_ZMOrigin[1] << "," << m_ZMOrigin[2] << std::endl;
    ...
    typename ImMaskType::SizeType imsize;
...
//          if((r-NumRows/2)*(r-NumRows/2) + (c - NumCols/2)*(c - NumCols/2) + (d - NumDeps/2)*(d - NumDeps/2) < rad2) {
//            idx[0] = r;
//            idx[1] = c;
//            idx[2] = d;

//            imLab->SetPixel(idx, 1);
//            }
//          }
//      }
//    }
    ...
    std::stringstream ss;
    std::string strIt, fnSave;
    ss.clear();
    ss << m_zmInter;
    strIt = ss.str();

    fnSave = "../../Results/Test/ZMIter_" + strIt + ".nrrd";
    typedef itk::ImageFileWriter< ImMaskType > WriterType;

    typename WriterType::Pointer writer = WriterType::New();
    writer->SetFileName(fnSave.c_str());
    writer->SetInput(imLab);
