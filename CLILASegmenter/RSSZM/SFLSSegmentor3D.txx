...
::setImage(typename ImageType::Pointer img)
{
    ...
    const double* spacing = img->GetSpacing().GetDataPointer();
...
double
CSFLSSegmentor3D< TPixel >
::computeAreaVolumeVariation(long ix, long iy, long iz) {
    ...
    double kappaNabla, AVRatio, AVRatio2, AVRatio3;


    kappaNabla = (dxx*(dy2 + dz2) + dyy*(dx2 + dz2) + dzz*(dx2 + dy2) - 2*dx*dy*dxy - 2*dx*dz*dxz - 2*dy*dz*dyz)\
            /(dx2 + dy2 + dz2 + vnl_math::eps);

    AVRatio = m_lz.size()/(double)(m_insideVoxelCount);

    AVRatio2 = AVRatio*AVRatio;
    AVRatio3 = AVRatio2*AVRatio;

    return 3*AVRatio2*kappaNabla + 2*AVRatio3*std::sqrt(dx2 + dy2 + dz2 + vnl_math::eps);
}
